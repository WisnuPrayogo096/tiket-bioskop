...
$result2 = mysqli_query($link, "SELECT * FROM pemesanan");
		...
		<table border = "1">
			...
			<?php
				while($row = mysqli_fetch_assoc($result2)){
					echo"<tr><td>";
					echo $row["no"]."</td><td>";
					$tempat_parts = explode('-', $row['tempat']);
					if (count($tempat_parts) > 1) {
						echo trim($tempat_parts[1]); // Menampilkan bagian setelah strip
					} else {
						echo $row['tempat']; // Jika tidak ada strip, tampilkan keseluruhan string
					}
					echo "</td><td>";
					...
					echo $row["payment"]."</td><td>";
					echo '<a href="deletePemesanan.php?no=' . $row["no"] . '" onclick="return confirm(\'Apakah Anda yakin ingin menghapus data?\')">Delete</a></td>';
					echo "</tr>";
				}
			?>
		</table>
